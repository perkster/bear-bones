...
function bb_nav_menu_link_class( $atts, $item, $args ) {
	...
	$classes = bb_nav_add_fa_icon_class_to_link( $classes, $item->classes );
	$atts["class"] = $classes;
    return $atts;
}

function bb_nav_add_fa_icon_class_to_link( $classes = null, $itemClasses = null ) {
	
	if( in_array ( 'bb-fa', $itemClasses ) ) {
		$i = 0;
		foreach( $itemClasses as $itemClass ) {
			if( $itemClass == 'bb-fa' ) $classes .= ' fa';
			if( strpos( $itemClass, 'b-fa-' ) ) $classes .= ' fa-' . substr( $itemClass, 6);
		}
