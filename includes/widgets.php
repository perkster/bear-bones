...
function bb_register_sidebars() {
	...
	//if( get_theme_mod ('topbar_widget') ) {
		register_sidebar(array(
				'id' => 'topbar-widget',
				'name' => 'Topbar Widget',
				'description' => __('This widget appears at the very top of the website.', 'bear-bones'),
	...
	if( get_theme_mod ('second_topbar_widget') ) {
		register_sidebar(array(
				...
				'description' => __('This widget appears at the very top of the website (after the topbar).', 'bear-bones'),
	...
	//if(get_theme_mod ('header_widget') ) {
		register_sidebar(array(
				'id' => 'header-widget',
				'name' => 'Header Widget',
				'description' => __('This widget appears in the header section.', 'bear-bones'),
	...
	//if( get_theme_mod ('pages_widget') ) {
		register_sidebar(array(
				...
				'description' => __('This widget appears in a sidebar on pages.', 'bear-bones'),
	...
	//if($bb_posts_widget == 'custom') {
		register_sidebar(array(
				'id' => 'posts-widget',
				'name' => 'Posts Sidebar',
				'description' => __('This widget appears in a sidebar on posts.', 'bear-bones'),
...
function bb_add_additional_widgets() {
	...
	$bb_addl_widgets = get_theme_mod('addl_widgets'); //prar($bb_addl_widgets);
	if($bb_addl_widgets) {
...
//function bb_display_widget( $widget, $extra_classes = null, $wrapper = false ) 
/**
 * Function to display widget
 * @param $widget text - name of widget
 * @param $args array
 *  'extra_classes' => additional classes to add to the widget containing div
 *  
 */
function bb_display_widget( $widget, $args = null ) {
	//prar($widget); prar($args);	
	if($widget) {
		if(isset( $args ) && is_array($args) ) extract ( $args ); 
		$widgetID = bb_widget_id( $widget );  prar($widgetID);
		if ( is_active_sidebar( $widgetID ) ) {
			$classes = isset($extra_classes) ? $widgetID . ' ' . $extra_classes : $widgetID;
			if( isset( $wrapper ) ) echo '<div class="' . $widgetID . '-wrapper">';
			echo '<div class="' . $classes . '">';
			dynamic_sidebar( $widgetID ); 
			echo '</div>';
			if( isset( $wrapper ) ) echo '</div>';
		}
	}
}
...
Class Bear_Bones_Recent_Comments_Widget extends WP_Widget_Recent_Comments {
	...
	function widget( $args, $instance ) {
		...
		if ( isset( $cache[ $args['widget_id'] ] ) ) {
 		...
 		$output = '';
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Recent Comments', 'bear-bones' ) : $instance['title'], $instance, $this->id_base );
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment) {
				$output .=  '<li class="recentcomments '.$id.'-widget__item widget__item">' . /* translators: comments widget: 1: comment author, 2: post link */ sprintf(_x('%1$s on %2$s', 'widgets', 'bear-bones'), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '" class="widget__link">' . get_the_title($comment->comment_post_ID) . '</a>') . '</li>';
			}
 		}
		$output .= '</ul>';
		$output .= $after_widget;
...
Class Bear_Bones_Archives_Widget extends WP_Widget_Archives {
	function widget( $args, $instance ) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Archives', 'bear-bones') : $instance['title'], $instance, $this->id_base);
		...
		if ( $d ) {
?>
		<select name="archive-dropdown" onchange='document.location.href=this.options[this.selectedIndex].value;'> <option value=""><?php echo esc_attr(__('Select Month', 'bear-bones')); ?></option> <?php wp_get_archives(apply_filters('widget_archives_dropdown_args', array('type' => 'monthly', 'format' => 'option', 'show_post_count' => $c))); ?> </select>
<?php
		} else {
?>
...
?>
			...
			<li class="<?php echo $id; ?>-widget__item widget__item"><a href="<?php bloginfo('rss2_url'); ?>" title="<?php echo esc_attr(__('Syndicate this site using RSS 2.0', 'bear-bones')); ?>"><?php _e('Entries <abbr title="Really Simple Syndication">RSS</abbr>', 'bear-bones'); ?></a></li>
			<li class="<?php echo $id; ?>-widget__item widget__item"><a href="<?php bloginfo('comments_rss2_url'); ?>" title="<?php echo esc_attr(__('The latest comments to all posts in RSS', 'bear-bones')); ?>"><?php _e('Comments <abbr title="Really Simple Syndication">RSS</abbr>', 'bear-bones'); ?></a></li>
			<li class="<?php echo $id; ?>-widget__item widget__item"><a href="<?php esc_attr_e( 'http://wordpress.org/', 'bear-bones' ); ?>" title="<?php echo esc_attr(__('Powered by WordPress, state-of-the-art semantic personal publishing platform.', 'bear-bones')); ?>"><?php
...
//@TODO: register widgets with theme prepended
?>