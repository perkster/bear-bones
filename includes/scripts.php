...
add_action('after_setup_theme', 'ps_footer_enqueue_scripts');

function bear_bones_scripts_get_filetime_version ( $filename = null, $fileLocation = null, $append = true ) {
	$fileTime = ''; 

	if( substr( $filename, 0, 1 ) != '/' ) $filename = '/' . $filename;
	if( file_exists( $fileLocation . $filename ) ){
		$fileTime = filemtime( $fileLocation . $filename);
		if( $append ) $filename .= '?' . $fileTime;
	}
	return $filename;
}
...
function bb_main_style ( $returnFile = false, $version = true) {
	...
	//if override stylesheet doesn't exist, set style to default
	...
	if( get_theme_mod( 'use_admin_style' )) {
		...
		// user is an admin		
			...
			if( file_exists ( $admin_style ) ) 
				$main_style = $admin_style;
			//check if file extension included
			if( strpos( $admin_style, '.' )  == 0  ){
				$admin_style .= '.css';
			}
	...
	if ( $version ) $main_style = bear_bones_scripts_get_filetime_version( $main_style, get_stylesheet_directory() );
