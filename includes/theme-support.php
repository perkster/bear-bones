...
$content_width = get_theme_mod( 'content_width' ) ? get_theme_mod( 'content_width' ) : 800;
...
function bear_bones_theme_support() {
	...
	register_nav_menus(
		array(
			'main-menu' => __( 'Main Menu', 'bear-bones' ),   // main menu
			'menu-social' => __( 'Social Menu', 'bear-bones' ) // used for floating social menus
		)
	);
...
add_filter('get_avatar','bb_change_avatar_css');

function bb_change_avatar_css($class) {
	$class = str_replace("class='avatar", "class='author_gravatar media__img avatar__img ", $class) ;
	return $class;
}

add_filter('comment_reply_link','bb_change_comment_reply_link_css');

function bb_change_comment_reply_link_css($class) {
	...
	<li <?php comment_class('comment'); ?>>
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment__body">
			<header class="comment__header">
				<?php printf(__('<cite class="fn comment__cite">%s</cite>', 'bear-bones'), get_comment_author_link()) ?> on <time datetime="<?php echo comment_time('Y-m-j'); ?>" class="comment__time"><?php comment_time(__('F jS, Y', 'bear-bones')); ?> </time> said:
			</header>	
			...
			<section class="comment__content ">					
				...
				<?php if ($comment->comment_approved == '0') : ?>
					<div class="alert info">
						<p><?php _e('Your comment is awaiting moderation.', 'bear-bones') ?></p>
					</div>
				<?php endif; ?>
			
				<?php comment_text() ?>
				<?php if ( is_user_logged_in() ) {  ?><span class="comment__link--edit"><?php edit_comment_link(__('(Edit)', 'bear-bones'),'  ','') ?></span> <?php } ?>
...
function bb_check_referrer() {
    if (!isset($_SERVER['HTTP_REFERER']) || $_SERVER['HTTP_REFERER'] == "") {
        wp_die( __('Please enable referrers in your browser, or, if you\'re a spammer, get out of here!', 'bear-bones') );
    }
}
add_action('check_comment_flood', 'bb_check_referrer');
...
if ( ! function_exists( 'bear_bones_entry_meta' ) ) {
	...
	function bear_bones_entry_meta() {
		if ( is_sticky() && is_home() && ! is_paged() )
			echo '<span class="featured-post">' . __( 'Sticky', 'bear-bones' ) . '</span>';

		if ( ! has_post_format( 'link' ) && 'post' == get_post_type() )
			bear_bones_entry_date();

		// Translators: used between list items, there is a space after the comma.
		$categories_list = get_the_category_list( __( ', ', 'bear-bones' ) );
		if ( $categories_list ) {
			echo '<span class="categories-links">' . $categories_list . '</span>';
		}

		// Translators: used between list items, there is a space after the comma.
		$tag_list = get_the_tag_list( '', __( ', ', 'bear-bones' ) );
		if ( $tag_list ) {
		...
		if ( 'post' == get_post_type() ) {
			printf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
				...
				esc_attr( sprintf( __( 'View all posts by %s', 'bear-bones' ), get_the_author() ) ),
				get_the_author()
			);
		}
	}
...
if ( ! function_exists( 'bear_bones_entry_date' ) ) {
	...
	function bear_bones_entry_date( $echo = true ) {
		if ( has_post_format( array( 'chat', 'status' ) ) )
			$format_prefix = _x( '%1$s on %2$s', '1: post format name. 2: date', 'bear-bones' );
		else
			$format_prefix = '%2$s';

		$date = sprintf( '<span class="date"><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a></span>',
			esc_url( get_permalink() ),
			esc_attr( sprintf( __( 'Permalink to %s', 'bear-bones' ), the_title_attribute( 'echo=0' ) ) ),
...
function bb_addtional_theme_image_sizes () {
	...
	bb_add_image();
	bb_add_image('page__thumbnail');
	bb_add_image('page__featured_image');
...
function bb_add_image ($theme_mod = 'post__thumbnail', $default = '150-15-crop') {
	...
	if($postFeaturedText) {
		...
		if(!isset($imgParts[1]) ) $imgParts = explode(',', $postFeaturedText);
		//check to make sure numbers are being passed
		//prar($imgParts);
		if( is_array( $imgParts ) ) {
...
function bear_bones_get_logo ( $atts = null ) {
	
	if( is_array($atts) ) extract ($atts);
	if( !isset( $src ) ) $src = get_theme_mod('site_logo'); //prar($src);
	$atts = array(
		'src' => $src,
		'alt' => get_theme_mod('logo_alt_text'),
		'class' => 'header__logo',
		'method' => get_theme_mod('logo_method'),
	);
	...
	if(get_theme_mod('header_logo_image')) {
		$logo_classes = "header__logo bb-svg";
	} else {
		$logo_classes = "header__logo";
	}*/
							//echo  '<img src="'. $logo .'" alt="' . $logo_alt . ' logo" class="' . $logo_classes . '">';
