...
function bb_page_nav_links () {
		wp_link_pages(array(
			'before' => '<p>' . __('Pages:', 'bear-bones'),
			'after' => '</p>',
			'next_or_number' => 'next_and_number',
			'nextpagelink' => __('Next', 'bear-bones'),
			'previouspagelink' => __('Previous', 'bear-bones'),
			'pagelink' => '%',
			'echo' => 1 )
		);
}
...
//@TODO fix bb_excerpt to accept $atts instead of explicit variables
function bb_excerpt($excerpt_length = null, $id = false, $echo = true, $excerpt_more = null ) {
         
    $excerpt = '';
	if ( !$excerpt_more ) $excerpt_more = get_theme_mod( 'posts__excerpt_read_more' );
	...
	//Strips tags, images and other shortcodes
	$excerpt = strip_tags( strip_shortcodes( $excerpt ) ); 
	//shortcodes not getting stripped correctly using built in function. Added custom preg_replace to do the job.
	//http://endlessgeek.com/2014/02/wordpress-strip-shortcodes-excerpts/
	$excerpt = preg_replace( '/\[[^\]]+\]/', '', $excerpt );
	
	if( !$excerpt_length ) $excerpt_length = get_theme_mod( 'posts__excerpt_length' );
	
	//Split exerpt into array to count words
	/* Old code to split into words... now using character count*/
	//$words = preg_split("/(?<=\w)\b\s*[!?.]*/", $excerpt, -1, PREG_SPLIT_NO_EMPTY);
	/*prar($words);
	...
	if( strlen( $excerpt) > $excerpt_length ) $excerpt = substr( $excerpt, 0, $excerpt_length ) . $excerpt_more;
